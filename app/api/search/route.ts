...
import { searchBrave } from "@/lib/search/client";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Perform search
    const result = await searchBrave(params);

    // Cache results (async, don't wait)
    setCachedResults(params, result).catch((error) => {
      console.error("[Search API] Failed to cache results:", error);
    });
