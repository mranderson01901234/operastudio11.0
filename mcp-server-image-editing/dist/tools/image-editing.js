...
export class ImageEditingTools {
     ...
     */
    async loadImage(pathOrBase64, isBase64) {
        if (isBase64) {
     ...
     */
    async crop(imageBuffer, x, y, width, height) {
     ...
     * Resize image
     */
    async resize(imageBuffer, width, height, maintainAspectRatio = true) {
     ...
     */
    async adjust(imageBuffer, options) {
     ...
     * Apply filter
     */
    async applyFilter(imageBuffer, filter, intensity) {
        let instance = sharp(imageBuffer);
     ...
     */
    async rotate(imageBuffer, angle, flip) {
        let instance = sharp(imageBuffer);
        if (flip) {
     ...
     * Convert format
     */
    async convertFormat(imageBuffer, format, quality = 80) {
        let instance = sharp(imageBuffer);
     ...
     */
    async generatePreviews(imageBuffer) {
